import type { Locale, MessagesDataset, Emoji } from 'emojibase';
 ...
 */
function getCdnUrls(version: string, locale: Locale) {
 ...
 */
function getEtags(locale: Locale): Promise<Array<string | null>> {
  const { emojisUrl, messagesUrl } = getCdnUrls('latest', locale);

  try {
    return Promise.all([getEtag(emojisUrl), getEtag(messagesUrl)]);
  } catch (error) {
 ...
 */
async function initDatabaseFromCdn(
  locale: Locale,
  factory: DataStoreFactory,
  customEmojis?: CustomEmoji[],
  existingDb?: DataStore
 ...
 * @returns a Promise that resolves to a fully populated database instance
 */
async function initDatabaseWithLocalData(
  locale: Locale,
  factory: DataStoreFactory,
  customEmojis: CustomEmoji[] = [],
...
export async function initDatabase(
  ...
  factory: DataStoreFactory,
  customEmojis?: CustomEmoji[],
  staticMessages?: MessagesDataset,
  staticEmojis?: Emoji[],
  existingDb?: DataStore
) {
