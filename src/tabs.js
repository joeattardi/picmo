...
export class Tabs {
  ...
  createTabs() {
    ...
    this.tabs = Object.keys(categoryIcons).map((category, index) => new Tab(categoryIcons[category], index + 1, this.setActiveTab));
  ...
  createTabBodies() {
    ...
    this.tabBodies = Object.keys(categoryIcons).map((category, index) => new TabBody(
      category,
      new EmojiContainer(emojiCategories[category], true, this.events).render(),
      index + 1
    ));
    ...
    this.tabBodies.forEach(tabBody => this.tabBodyContainer.appendChild(tabBody.render()));
